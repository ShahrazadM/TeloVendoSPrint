...
-- llenado de tabla Cliente con 5 clientes: Camila, Carlos, Danilo, Javiera y Javier para probar la base de datos.
...
VALUES ('Javier', 'García', 'Marquez', '985698785', 'Santa Laura 566', 'Iquique', 'javiergarcia@gmail.com', '2024-02-17');
 ...
 -- -- Llenado tabla Proveedores con datos de 5 proveedores, con los siguientes datos; nombre representante legal, nombre corporativo, teléfono1, teléfono2, categoria producto, correo para enviar factura.
...
-- llenado de tabla producto con 10 Productos.
...
-- se rectifico categoria.
UPDATE `telovendo1`.`producto` SET `category` = 'Electrónica y computacion' WHERE (`idproducto` = '1');
UPDATE `telovendo1`.`producto` SET `category` = 'Electrónica y computacion' WHERE (`idproducto` = '2');
UPDATE `telovendo1`.`producto` SET `category` = 'Electrónica y computacion' WHERE (`idproducto` = '5');

-- Llenado tabla vendedor con 10 vendedores.
...
-- La que mas se repite es categoria Electronica, se repite 3 veces.
...
-- El producto con mayor stock 'Mochila North Face Recon', '80 unidades'
...
-- El mas comun en nuestra tienda es 'Negro', '4'
...
-- El proveedor con menos stock es 'Carlos Sánchez', '30 unidades'
...
ORDER BY menor_stock;
 ...
 -- Con este codigo realizamos una actualización en la tabla "producto". Mostrandonos el que mas se repetia de la categoria.

UPDATE producto
SET category = 'Electrónica y computacion'
